...
def binary_loss(ytrue, ypred):
    ...
    z = [
        -(float(ytrue[i]) * math.log(ypred[i]))
        - ((1 - float(ytrue[i])) * math.log(1 - ypred[i]))
...
def update_model(scaled_data):
    ...
    xtrain, xtest, ytrain, ytest = train_test_split(
        scaled_data, raw_data[21], 0.7
    )  # 70% data used for training, can change this
    # Xtrain data
    # Train and Build model using Logistic Regression
    w, b, loss_train, ypred_train = logistic_regressor(
        xtrain, ytrain, 0.1, 100
    )  # 1000 epoch, learning rate =0.1
...
i2c = machine.I2C(
    0,
    scl=machine.Pin(17, pull=machine.Pin.PULL_UP),
    sda=machine.Pin(16, pull=machine.Pin.PULL_UP),
    freq=400000,
)
...
while True:
    ...
    if slice_count == 12:
        ...
        x_mmd = math.sqrt(
            ...
            + (x_arg_max - x_arg_min) * (x_arg_max - x_arg_min)
        )
        y_mmd = math.sqrt(
            (y_max - y_min) * (y_max - y_min)
            + (y_arg_max - y_arg_min) * (y_arg_max - y_arg_min)
        )
        z_mmd = math.sqrt(
            (z_max - z_min) * (z_max - z_min)
            + (z_arg_max - z_arg_min) * (z_arg_max - z_arg_min)
        )
        ...
        data = [
            [(x_min - X_min_mean) / X_min_std],
            [(y_min - Y_min_mean) / Y_min_std],
            [(z_min - Z_min_mean) / Z_min_std],
            [(x_max - X_max_mean) / X_max_std],
            [(y_max - Y_max_mean) / Y_max_mean],
            [(z_max - Z_max_mean) / Z_max_mean],
            [(x_std - X_std_mean) / X_std_std],
            [(y_std - Y_std_mean) / Y_std_std],
            [(z_std - Z_std_mean) / Z_std_mean],
            [(x_mean - X_mean_mean) / X_mean_std],
            [(y_mean - Y_mean_mean) / Y_mean_std],
            [(z_mean - Z_mean_mean) / Z_mean_std],
            [x_slope],
            [y_slope],
            [z_slope],
            [(x_zc - X_zc_mean) / X_zc_std],
            [(y_zc - Y_zc_mean) / Y_zc_std],
            [(z_zc - Z_zc_mean) / Z_zc_std],
            [(x_mmd - X_mmd_mean) / X_mmd_std],
            [(y_mmd - Y_mmd_mean) / Y_mmd_std],
            [(z_mmd - Z_mmd_mean) / Z_mmd_mean],
        ]
    ...
    pitch = (
        180 * math.atan(xAccl / math.sqrt(yAccl * yAccl + zAccl * zAccl)) / (math.pi)
    )
